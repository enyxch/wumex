...
- (void)panBegan:(UILongPressGestureRecognizer *)gestureRecognizer;

- (void)panMoved:(UILongPressGestureRecognizer *)gestureRecognizer;

- (void)panEnded:(UILongPressGestureRecognizer *)gestureRecognizer;
...
- (id)initWithImage:(UIImage *)image 
    ...
    self = [self  initWithImage:image
                     ...
                     goodFrames:@[[NSValue valueWithCGRect:endFrame]]
                      badFrames:nil
                    andDelegate:nil];
...
- (id)initWithImage:(UIImage *)image
    ...
    UILongPressGestureRecognizer* panGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(panDetected:)];
...
- (void)setDelegate:(id<TKDragViewDelegate>)delegate{
    if (delegate != delegate_) {
        ...
        delegateFlags_.dragViewDidMove              = [delegate_ respondsToSelector:@selector(dragViewDidMove:)];
        delegateFlags_.dragViewWillMove              = [delegate_ respondsToSelector:@selector(dragViewWillMove:)];
...
- (void)panDetected:(UILongPressGestureRecognizer*)gestureRecognizer{
...
- (void)panBegan:(UILongPressGestureRecognizer*)gestureRecognizer{
	...
	if (!isDragging_ && !isAnimating_) {
        ...
        const CGPoint ptInSuperview = [gestureRecognizer locationInView:[self parentContainer]];

        startLocationInParent = ptInSuperview;
        startLocation = pt;
        _offsetStartLocation = CGPointMake(self.frame.origin.x - ptInSuperview.x,
                                          self.frame.origin.y - ptInSuperview.y);
        ...
        if (self.dragsAtCenter)
            self.frame = CGRectMake(self.frame.origin.x + startLocation.x - self.frame.size.width / 2,
                                    self.frame.origin.y + startLocation.y - self.frame.size.height / 2,
...
- (void)panMoved:(UILongPressGestureRecognizer*)gestureRecognizer{
    ...
    if (delegateFlags_.dragViewWillMove) {
        [self.delegate dragViewWillMove:self];
    }
        
    CGPoint pt = [gestureRecognizer locationInView:[self parentContainer]];
    self.frame = CGRectMake(pt.x + _offsetStartLocation.x,
                           pt.y + _offsetStartLocation.y,
                           self.frame.size.width,
                           self.frame.size.height);
    if (self.isHorizontalDraggable) {
        self.frame = CGRectMake(pt.x + _offsetStartLocation.x,
                                startLocationInParent.y + _offsetStartLocation.y,
                                self.frame.size.width,
                                self.frame.size.height);
    }
    // Is over start frame
    ...
    if (delegateFlags_.dragViewDidMove) {
        [self.delegate dragViewDidMove:self];
    }
