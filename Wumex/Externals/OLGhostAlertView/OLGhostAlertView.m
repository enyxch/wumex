...
- (void)show
{
    ...
    if ([parentController isKindOfClass:[UITabBarController class]]) {
        parentController = ((UITabBarController*)parentController).selectedViewController;
    }
    
    if ([parentController isKindOfClass:[UINavigationController class]]) {
        parentController = ((UINavigationController*)parentController).topViewController;
    }
...
{
    [self hideOtherOLGAVinView:view];
...
- (void)showInView:(UIView *)view withOffset:(CGPoint)offset
{
    [self hideOtherOLGAVinView:view];
...
- (void)hideOtherOLGAVinView:(UIView*)view
{
    for (UIView *subview in [view subviews]) {
        if (subview.subviews.count > 0) {
            [self hideOtherOLGAVinView:subview];
        }
...
- (void)setStyle:(OLGhostAlertViewStyle)style
{
    ...
    if (style == OLGhostAlertViewStyleLight) {
        ...
        backgroundColor = [UIColor colorWithWhite:0 alpha:.65];
